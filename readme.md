...
Lossless Scaling(LS)을 목표로 개발을 진행하였으나, 분석 결과 LS는 실시간으로 고비용의 딥러닝 추론을 수행하는 방식이 아니라, 사전에 딥러닝으로 학습된 패턴을 경량화하여 제한적으로 적용하는 구조임을 확인하였다. 이에 따라 현재 진행 중인 프로젝트 역시 구조를 일부 수정할 경우, LS와 유사한 방식으로 구현이 가능할 것으로 판단된다.
